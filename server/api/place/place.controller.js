...
exports.insert = function (collection) {
  Place.insert(collection, function (collection) {
    res.status(200)
      .send({
        details: 'Collection added to db',
  ...
  Place.edit(placeID, attributes)
    ...
    .catch(function (err) {
      res.status(400)
        .send({
          error: 'Bad Request',
          details: 'place cannot be edited, ' + err.message
        });
    });
};
  ...
  Place.del(placeID)
    ...
    .catch(function (err) {
      res.status(400)
        .send({
          error: 'Bad Request',
          details: 'place cannot be deleted, ' + err.message
        });
    });
};
